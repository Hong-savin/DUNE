...
import torch
import os
selected_labels = ["door_nock", "glass_shatter", "car_horn",
                   "dog_bark", "drilling", "nothing", "siren", "nothing2"]
...
def continuous_sound_prediction(model, device, transformation, sample_rate, target_sample_rate):
    ...
    with torch.no_grad():  # deactivate autograd engine to reduce memory usage and speed up computations
        ...
        # apply sigmoid to output (for indivisual points)
    ...
    try:  # if you have something to reduce
        ...
        probabilities[0, x_index] = max(
            0.0, probabilities[0, x_index].item() - change_probability)
    except:  # if you dont
        pass
...
class SoundAnalysis(QThread):
    ...
    # Add a float type for the probability
    ...
    def run(self):
        ...
        while True:
            ...
            prob_strs = [f"{label} {probabilities[0, idx].item():.2%}" for idx,
                         label in enumerate(selected_labels)]
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.sound_analysis = SoundAnalysis(
            model, device, transformation, SAMPLE_RATE)
    ...
    def updateLabel2(self, predicted_label):
        ...
        image_folder_path = os.path.join(
            current_path, relative_image_folder_path)
        full_file_name = os.path.join(
            image_folder_path, f"{predicted_label}.gif")
    ...
    def updateLabel(self, predicted_label, predicted_confidence):
        ...
        self.label.setText(
            f"{predicted_label}  {predicted_confidence*100:.2f}%")
        # print(predicted_label)
                ...
                if red_button_state != self.prev_red_button_state:
                    ...
                    if red_button_state != GPIO.input(self.red_button_pin):
                        ...
                        if not self.vibration_on:
                            ...
                            self.adjust_vibration_intensity(
                                self.red_button_pin)
                        else:
